...
// POST /auth/login - Driver/Admin login
router.post('/login', 
  authLimiter,
  loginValidation,
  handleValidationErrors,
  async (req, res) => {
    try {
      ...
      // Check in users table (drivers and admins)
      const userQuery = `
...
// POST /auth/register-driver - Register new driver (admin only)
  ...
  async (req, res) => {
    ...
    try {
      ...
      const userResult = await client.query(userQuery, [
        email, passwordHash, 'driver', name, phone
      ]);
    ...
    try {
      ...
      if (user.role === 'driver') {
        profile.driver = {
          ...
          assignedVehicle: user.assigned_vehicle_id ? {
            id: user.assigned_vehicle_id,
...
// POST /auth/logout - Logout (optional: token blacklisting)
router.post('/logout',
  authenticateToken,
  (req, res) => {
